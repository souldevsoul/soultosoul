...
export default function WaveAnimation() {
  ...
  useEffect(() => {
    // Delay visibility for animation timing
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Draw spinning circular lines that follow mouse
      const spinCount = 5
      for (let i = 0; i < spinCount; i++) {
        const angle = (time * 0.5) + (i * Math.PI * 2 / spinCount)
        ...
        x += (mousePos.x - x) * mouseEffect * 0.3
        y += (mousePos.y - y) * mouseEffect * 0.3

        ctx.beginPath()
        ctx.arc(x, y, 3 + mouseEffect * 2, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(0, 0, 0, ${0.7 - i * 0.1})`
        ctx.fill()
      }

      // Draw pulsing dots along main wave that react to mouse
      const dotCount = 8
  ...
  return (
    <canvas
      ...
      width={1200}
      height={600}
      className={`absolute top-1/2 right-[-15%] -translate-y-1/2 transition-opacity duration-1000 pointer-events-none ${
